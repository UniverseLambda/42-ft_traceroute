...
/*   Updated: 2023/08/04 17:32:49 by clsaad           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static bool	is_buffer_sufficient(const char *buffer, size_t size,
				const char *iphdr)
{
...
static size_t	get_packet_type(const t_icmphdr *icmphdr)
{
...
t_icmp_res	parse_icmp_resp(const char *buf, size_t size, int start_port,
				int end_port)
{
	const t_icmphdr	*icmphdr = (t_icmphdr *)(buf + get_ipdata_offset(buf));
	const char		*orig_iphdr = buf + get_ipdata_offset(buf) + 8;
	const char		*orig_data;
	t_icmp_res		result;

	ft_memset(&result, 0, sizeof(result));
	if (!is_buffer_sufficient(buf, size, orig_iphdr))
		return (result);
	orig_data = orig_iphdr + get_ipdata_offset(orig_iphdr);
	result.packet_type = get_packet_type(icmphdr);
	if (result.packet_type == ICMP_INVALID_TYPE)
		return (result);
	...
	ft_memcpy(&(result.emitter.s_addr), buf + 12, 4);
